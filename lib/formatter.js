...
/**
 * Resolve a subject template string by replacing supported placeholders.
 *
 * Supported placeholders:
 *   {count}     — total occurrences (sum of error.count fields)
 *   {errors}    — number of distinct error records
 *   {timestamp} — ISO 8601 timestamp
 *
 * Unknown placeholders are left unchanged.
 *
 * @param {string} template
 * @param {{ count: number, errors: number, timestamp: string }} vars
 * @returns {string}
 */
function resolveSubject(template, { count, errors, timestamp }) {
  return template
 ...
 * @param {string}   [options.subjectTemplate] - subject template with {count}, {errors}, {timestamp} placeholders
 * @returns {{ subject: string, html: string }}
 */
function formatHtmlEmail(errors, options = {}) {
  ...
  const subjectTemplate = options.subjectTemplate ||
    '[CAP Error Outbox] {count} occurrence(s) in {errors} error(s) — {timestamp}';
  const subject = resolveSubject(subjectTemplate, { count: total, errors: errors.length, timestamp });
...
module.exports = { formatHtmlEmail, escapeHtml, resolveSubject };
