...
async function runBatch(config, provider) {
  ...
  let subject, html;
  try {
    ({ subject, html } = formatHtmlEmail(errors, { subjectTemplate: config.mail && config.mail.subject }));
  } catch (err) {
  ...
  // ── 6. Cleanup — delete old sent records beyond retention window ──────────
  const retentionDays = config.retention && config.retention.days;
  if (retentionDays > 0) {
    try {
      const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000).toISOString();
      const deleted = await db.run(
        DELETE.from(ENTITY).where({ sent: true }).and('lastSeen <', cutoff)
      );
      if (deleted > 0) {
        console.info(`[cds-error-outbox] Cleanup — deleted ${deleted} sent record(s) older than ${retentionDays} day(s).`);
      }
    } catch (err) {
      console.error('[cds-error-outbox] Cleanup failed:', err.message);
    }
  }
}
